...
const platforms = {
  facebook:  { path: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' },
...
export default function Footer({ siteName, mobile, email, address, copyrightText, socialLinks }) {
  ...
  const brandName = siteName || DEFAULT_BRAND;
      ...
      <div className="mx-auto max-w-[1440px] px-page-margin-mobile py-16 text-center sm:px-6 sm:text-left lg:px-page-margin-desktop">
        <div className="grid gap-10 text-center sm:grid-cols-2 sm:text-left lg:grid-cols-4">
          ...
          <div className="space-y-4">
            ...
            {address && <p className="text-sm leading-relaxed text-white/60">{address}</p>}
          ...
          {/* Shop links */}
          <div className="space-y-4">
            <h4 className="font-label-caps text-[0.65rem] tracking-[0.15em] text-white/40">Shop</h4>
          ...
          {/* Company links */}
          <div className="space-y-4">
            <h4 className="font-label-caps text-[0.65rem] tracking-[0.15em] text-white/40">Company</h4>
            <ul className="space-y-3 text-sm">
          ...
          {/* Newsletter + Socials */}
          <div className="space-y-4">
            <h4 className="font-label-caps text-[0.65rem] tracking-[0.15em] text-white/40">Newsletter</h4>
            ...
            >
              <input
                ...
                className="min-w-0 flex-1 rounded-lg border border-white/20 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/40 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
              />
              <button
                type="submit"
                className="shrink-0 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-on-primary transition hover:bg-primary/90"
              >
                Subscribe
              </button>
            </form>
            ...
            <div className="flex justify-center gap-2 pt-2 sm:justify-start">
              {socialLinks?.filter((l) => l.isActive).map((link) => {
                ...
                return (
                  ...
                  >
                    {platform ? (
                      <svg className="h-4 w-4 text-white/60 transition group-hover:text-editorial-ink" viewBox="0 0 24 24" fill="currentColor">
                        <path d={platform.path} />
                      </svg>
                    ) : (
                      <span className="material-symbols-outlined text-[18px] text-white/60 transition group-hover:text-editorial-ink">
                        language
                      </span>
                    )}
                  </a>
