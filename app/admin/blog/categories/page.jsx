              ...
              <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors dark:hover:bg-slate-700/30">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {cat.image ? (
                      <img src={cat.image} alt={cat.title} className="h-9 w-9 shrink-0 rounded-lg border border-slate-200 object-cover dark:border-slate-700" />
                    ) : (
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-sm font-semibold text-slate-400 dark:bg-slate-900/50 dark:text-slate-500">{cat.title.charAt(0).toUpperCase()}</div>
                    )}
                    <span className="min-w-0 font-medium text-slate-900 dark:text-white">{cat.title}</span>
                  </div>
                </td>
